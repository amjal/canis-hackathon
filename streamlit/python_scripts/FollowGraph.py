...
def plot_network_graph(user_parent_entity, following_parent_entity):
    df = pd.read_csv('./network.csv')
    df = df[df['user_parent_entity'].isin(user_parent_entity) & df['following_parent_entity'].isin(following_parent_entity)]
    ...
    parent_entity_color_map = {entity: get_pleasing_random_color() for entity in set(df['user_parent_entity'].unique()).union(set(df['following_parent_entity'].unique()))}

    node_in_degree = dict(G.in_degree())
    max_in_degree = max(node_in_degree.values())
    node_size = [5 + 10 * (node_in_degree[node] / max_in_degree) for node in G.nodes()]  # Scale node sizes based on in-degree
